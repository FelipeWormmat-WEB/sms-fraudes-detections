...
- Para configurações de ambiente e credenciais, verifique gateway-service/app/core/config.py e variáveis consumidas pelo docker-compose.yml.

## Como usar este documento

- Use a seção "Estrutura" como mapa para localizar rapidamente arquivos e entender responsabilidades.
- Consulte as descrições por arquivo para identificar pontos de extensão (novos endpoints, mudanças de modelo, migrações, etc.).
- Para execução/desenvolvimento, siga os Dockerfiles e o docker-compose.yml ou scripts/padrões de cada subprojeto (Node/Yarn/NPM para o frontend; pip/venv para o gateway-service; `go run` para os serviços Go).
